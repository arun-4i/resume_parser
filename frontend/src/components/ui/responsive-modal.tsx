  ...
  ),
  {
    variants: {
      side: {
        ...
        bottom:
          "dark:bg-[#0c0c0c] inset-x-0 bottom-0 border-t lg:h-fit max-h-[90%] rounded-t-xl data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
    ...
    defaultVariants: {
      side: "bottom", // Default to bottom for small screens
    },
  }
);
...
>(({ side = "bottom", className, children, ...props }, ref) => (
  <ResponsiveModalPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "lg:data-[state=open]:slide-in-from-top lg:data-[state=closed]:slide-out-to-top", // Ensure this part is centered on large screens
        side === "bottom" &&
          "data-[state=open]:slide-in-from-bottom data-[state=closed]:slide-out-to-bottom", // For small screens (open from bottom)
